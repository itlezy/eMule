...
**Action:** Remove the commented lines entirely.
...
**Action:** Delete the declaration and definition.
...
**Action:** Delete all `#if 0` ... `#endif` blocks.
...
**Action:** Once minimum supported client version can be set to v2-capable, remove all `m_bySourceExchange1Ver` branches and `uSourceExchange1Ver` constant.
...
**Action:**
1. Remove `OP_PEERCACHE_*` handlers entirely (defunct — no network infrastructure remains).
...
**Key concern — `EncryptedStreamSocket.cpp`:** The comment "must be a bug" appears next to several `ASSERT(0)` calls. In a release build these become silent no-ops that may leave the socket in a corrupted state. These should be converted to proper error handling with `OnError()` and disconnect.
...
**Action:** After confirming the minimum supported config file age, remove these legacy reads.
...
**Action:** Remove all `// deadlake PROXYSUPPORT` attribution comments. The proxy support code itself is functional; only the attribution noise needs to go.
...
**Action:** Remove the Windows 95 / NT4 conditional block.
...
| File | Issues |
|------|--------|
| `srchybrid/BaseClient.cpp` | 17× ASSERT(0), Source Exchange v1 dead branches, unused member casts, obsolete mod comments |
| `srchybrid/EncryptedStreamSocket.cpp` | 14× ASSERT(0) with "must be a bug" comments — should be real error handling |
| `srchybrid/ArchiveRecovery.cpp` | 14× ASSERT(0) including acknowledged FIXME, partial feature |
| `srchybrid/DownloadClient.cpp` | 12× ASSERT(0), Source Exchange v1/v2 branching |
| `srchybrid/ListenSocket.cpp` | 10× ASSERT(0), defunct opcode handlers |
...
1. **Delete all `#if 0` blocks** — Confirmed dead, no compilation risk, ~300–400 lines gone.

2. **Remove `OP_PEERCACHE_*` handlers** in `ListenSocket.cpp` — Infrastructure is defunct network-wide.

3. **Remove Source Exchange v1 branches** — Once minimum client floor can be set to v2-capable. Simplifies ~200–300 lines in `BaseClient.cpp` and `DownloadClient.cpp`.

4. **Remove `deadlake PROXYSUPPORT` attribution comments** — 20+ instances, comments only, zero risk.

5. **Remove Windows 95 check** in `OtherFunctions.cpp:624` — Dead on any supported OS.

### Priority 2 — Medium Effort

6. **Convert `ASSERT(0)` + "must be a bug" paths in `EncryptedStreamSocket.cpp` to `OnError()`** — In release builds these silently fail; they should disconnect cleanly.

7. **Replace `ASSERT(0); // FIXME` in `ArchiveRecovery.cpp:233`** with a graceful error return.

8. **Remove Kademlia v1 residual comments and dead branches** in `KademliaUDPListener.cpp`.
...
11. **Remove obsolete `.ini` key loading** in `Preferences.cpp` once migration window is closed.
...
14. **Remove never-used `WebServer.h:_GetIsTempDisabled()`**.

15. **Remove commented-out radio handler declarations** in `AddSourceDlg.h`.
