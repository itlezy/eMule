...
1. [Crypto++ (cryptopp)](#1-crypto-cryptopp)
2. [Mbed TLS (mbedtls)](#2-mbed-tls-mbedtls)
3. [id3lib](#3-id3lib)
4. [miniupnpc (miniupnp)](#4-miniupnpc-miniupnp)
5. [zlib](#5-zlib)
6. [ResizableLib](#6-resizablelib)
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **Crypto++** | BREAKS | BREAKS | BREAKS | — | — | — | — | — |
| **Mbed TLS** | — | — | — | degrades (HTTP) | BREAKS | — | — | — |
| **id3lib** | — | — | — | — | — | — | removed | — |
| **miniupnpc** | — | — | — | — | — | — | — | manual only |
| **zlib** | degrades | degrades | degrades | degrades | — | — | — | — |
| **ResizableLib** | — | — | — | — | — | non-resizable | — | — |

### Removal difficulty

| Dependency | Effort | Risk | Recommended action |
|---|---|---|---|
| **id3lib** | Low | Very low | Remove or replace with taglib / Windows Shell IPropertyStore |
| **miniupnpc** | Very low | Very low | Switch to `UPnPImplWinServ` (already in codebase) |
| **ResizableLib** | Medium (31 files) | Low | Inline into source tree, or drop resizable dialogs |
| **Mbed TLS** | High | Low | Keep; or disable web server / consolidate with Crypto++ |
| **zlib** | Medium | Medium | Keep; or replace with miniz for same API |
| **Crypto++** | Very high | Critical | Keep; or consolidate all crypto onto Mbed TLS (long-term) |

### Priority order for reduction

If the goal is reducing the dependency count with the least disruption:

1. **miniupnpc** — one-line switch to the Windows-native UPnP fallback that already exists
2. **id3lib** — remove two isolated code blocks; MP3 metadata display disappears gracefully
3. **ResizableLib** — inline the library source into the tree; zero runtime change, submodule gone
4. **zlib** — replace with miniz; API-compatible, no separate build needed
5. **Mbed TLS** — only if the web server feature is being dropped, or as part of a full crypto consolidation
6. **Crypto++** — only as part of a multi-month effort to migrate all crypto call sites to Mbed TLS
