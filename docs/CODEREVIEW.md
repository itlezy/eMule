...
### 1. `CaptchaGenerator.cpp` — SelectObject called before NULL check
...
### 2. `CaptchaGenerator.cpp` — `rand() & 8` bimodal jitter
...
### 3. `Ring.h` — `m_pTail` initialized to `&buffer[-1]` (undefined behavior)
...
### 4. `Ring.h` — `operator[]` has no bounds check against `Count()`
...
### 5. `CreditsThread.cpp` — mask bitmap changed from 1-bit monochrome to screen color depth
...
### 6. `WebSocket.cpp` — `ULONGLONG` → `UINT` silent truncation in `Read()`
...
### 7. `emule.vcxproj` — `MBEDTLS_ALLOW_PRIVATE_ACCESS` is technical debt
...
|---|------|----------|--------|
| 1 | `CaptchaGenerator.cpp` | **CRITICAL** | ASSERT after SelectObject with potentially NULL handles; resource leak in release builds |
| 2 | `CaptchaGenerator.cpp` | **CRITICAL** | `rand() & 8` — bimodal output (0 or 8 only), not uniform 0–7 range |
| 3 | `Ring.h` | **CRITICAL** | `m_pTail = &buf[-1]` on construction; `m_pTail = m_pEnd` after RemoveAll() — both UB pointers |
| 4 | `Ring.h` | **CRITICAL** | `operator[]` has no bounds check against `Count()` — silent stale data reads |
| 5 | `CreditsThread.cpp` | **CRITICAL** | 1-bit monochrome mask replaced with color-depth bitmap — breaks compositing if used as BitBlt mask |
| 6 | `WebSocket.cpp` | **HIGH** | `ULONGLONG` → `UINT` silent truncation in `CFile::Read()`; `fileLen + 1` can overflow `size_t` |
| 7 | `emule.vcxproj` | **HIGH** | `MBEDTLS_ALLOW_PRIVATE_ACCESS` — brittle migration bridge, breaks on next MbedTLS restructure |
