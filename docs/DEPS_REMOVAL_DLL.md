...
|---|---------|---------|-----------|---------|
| 1 | **Crypto++** | 8.9.0 | Static `.lib` | RSA/AES/MD5/hash — client credits, crypto ops |
| 2 | **id3lib** | 3.9.1 | Static `.lib` | ID3 tag read/write for media files |
| 3 | **miniupnpc** | 2.3.3 | Static `.lib` | UPnP port mapping / NAT traversal |
| 4 | **ResizableLib** | master | Static `.lib` | MFC resizable dialogs |
| 5 | **zlib** | 1.3.2 | Static `.lib` (CMake) | Deflate/gzip compression |
| 6 | **MbedTLS** | 4.0.0 | Static `.lib` (6-part CMake) | TLS 1.3, X.509, WebSocket security |
| 7 | **TF-PSA-Crypto** | 1.0.0 | Static `.lib` (coupled to MbedTLS) | PSA Crypto API layer under MbedTLS |
...
## 6. MbedTLS 4.0.0 + TF-PSA-Crypto 1.0.0
...
| Library | Version | DLL Available | Feasibility | Effort | Verdict |
|---------|---------|--------------|-------------|--------|---------|
| **miniupnpc** | 2.3.3 | Yes (official) | High | Low | **DO IT** |
| **zlib** | 1.3.2 | Yes (official) | High | Low | **DO IT** |
| **Crypto++** | 8.9.0 | Yes (official) | Medium | Medium | Consider — test ABI |
| **id3lib** | 3.9.1 | No (dead upstream) | Low | Very High | **Keep static** |
| **ResizableLib** | master | No | None | N/A | **Incompatible** |
| **MbedTLS + TF-PSA** | 4.0.0 / 1.0.0 | Yes but complex | Very Low | Very High | **Keep static** |
...
Blocked until `MBEDTLS_ALLOW_PRIVATE_ACCESS` usage is removed from eMule source. File a
