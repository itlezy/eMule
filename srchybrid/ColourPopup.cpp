...
void CColourPopup::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_DOWN) {
		if (row == DEFAULT_BOX_VALUE)
			...
			row = col = (m_strDefaultText.GetLength() ? DEFAULT_BOX_VALUE : 0);
		else {
			++row;
			if (GetIndex(row, col) < 0) {
				if (m_strCustomText.GetLength())
					row = CUSTOM_BOX_VALUE;
				else
					row = m_strDefaultText.GetLength() ? DEFAULT_BOX_VALUE : 0;
				col = row;
			}
		}
...
void CColourPopup::FindCellFromColour(COLORREF crColour)
{
	...
	m_nChosenColourSel = m_strCustomText.IsEmpty() ? INVALID_COLOUR : CUSTOM_BOX_VALUE;
}
...
void CColourPopup::DrawCell(CDC *pDC, int nIndex)
{
	...
	if (m_strCustomText.GetLength() && nIndex == CUSTOM_BOX_VALUE) {
		...
		// Draw button
		b = (m_nCurrentSel == nIndex);
		if (b || m_nChosenColourSel == nIndex)
			pDC->DrawEdge(TextButtonRect, b ? BDR_RAISEDINNER : BDR_SUNKENOUTER, BF_RECT);
	...
	// fill background
	bool b = (m_nChosenColourSel == nIndex && m_nCurrentSel != nIndex);
	pDC->FillSolidRect(rect, ::GetSysColor(b ? COLOR_3DHILIGHT : COLOR_3DFACE));

	// Draw button
	b = (m_nCurrentSel == nIndex);
	if (b || m_nChosenColourSel == nIndex)
		pDC->DrawEdge(rect, b ? BDR_RAISEDINNER : BDR_SUNKENOUTER, BF_RECT);
