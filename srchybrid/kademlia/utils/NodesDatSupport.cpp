...
namespace Kademlia
{
	...
	{
		if (::GetFileAttributes(pszTargetFilename) != INVALID_FILE_ATTRIBUTES) {
			if (::ReplaceFile(pszTargetFilename, pszSourceFilename, NULL, REPLACEFILE_IGNORE_MERGE_ERRORS, NULL, NULL))
				return true;
		} else if (::MoveFileEx(pszSourceFilename, pszTargetFilename, MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH)) {
			return true;
		}

		const DWORD dwError = ::GetLastError();
		if (::MoveFileEx(pszSourceFilename, pszTargetFilename, MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
			return true;
