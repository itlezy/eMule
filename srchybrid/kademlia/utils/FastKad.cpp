...
namespace Kademlia
{
	...
	void CFastKad::SaveNodesMetadata(LPCTSTR pszFilename, const std::vector<NodeKey> &knownNodes) const
	{
		...
		if (records.empty()) {
			LongPathSeams::DeleteFile(pszFilename);
			return;
		}

		FILE *pFile = LongPathSeams::OpenFileStreamLongPath(pszFilename, GENERIC_WRITE, FILE_SHARE_READ, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, _O_WRONLY | _O_BINARY, "wb");
		if (pFile == NULL)
			return;
