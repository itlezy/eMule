...
void CSearchParamsWnd::UpdateControls()
{
	...
	m_ctlOpts.SetItemData(orAvailability, FALSE);
	m_ctlOpts.SetItemData(orExtension, FALSE);
	m_ctlOpts.SetItemData(orCompleteSources, static_cast<DWORD_PTR>(iMethod == SearchTypeKademlia)); //ed2k only
	m_ctlOpts.SetItemData(orCodec, FALSE);
	m_ctlOpts.SetItemData(orBitrate, FALSE);
	m_ctlOpts.SetItemData(orLength, FALSE);
