...
/** @brief Runtime loader for the required MediaInfo.dll dependency. */
class CMediaInfoDLL
{
public:
	...
	CString GetI(void *Handle, MediaInfo_stream_C StreamKind, size_t StreamNumber, size_t Parameter, MediaInfo_info_C KindOfInfo)
	{
		if (!m_pfnMediaInfo_GetI)
			return CString();
		return CString((*m_pfnMediaInfo_GetI)(Handle, StreamKind, StreamNumber, Parameter, KindOfInfo));
	}
...
static CMediaInfoDLL theMediaInfoDLL;

/** @brief Appends the red warning used when the extension does not match the parsed container. */
...
/** @brief Extracts audio/video metadata through the required MediaInfo.dll runtime. */
		...
		if (bFullInfo && !strExtension.IsEmpty()) {
			...
			if (!str.IsEmpty()) {
				...
				for (int iPos = 0; iPos >= 0;) {
					...
					if (!strFmtExt.IsEmpty() && strFmtExt == strExtension) {
						bFoundExt = true;
						break;
					}
				}
