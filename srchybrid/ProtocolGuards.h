...
/**
 * Reproduces the legacy packet payload calculation used by the oracle branch.
 */
...
/**
 * Reproduces the legacy TCP receive-buffer size gate used by the oracle branch.
 */
inline bool CanReadTcpPacketPayload(uint32 packetlength, size_t readBufferSize)
{
	return static_cast<size_t>(packetlength - 1) <= readBufferSize;
}
...
/**
 * Reproduces the legacy client and server tag-count handling without a hostile-count guard.
 */
...
/**
 * Reproduces the legacy server UDP receive path, which did not reject short payloads
 * before touching the protocol and opcode bytes.
 */
